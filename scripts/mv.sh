...
echo "This command, like most that involve modifying files, may require elevated permissions (sudo) to depending on your user permissions."
echo -en "\n"
...
"\tmv -i\t\t\t\tmakes the command ask the user for confirmation before moving and overwriting a file."
"\t\t\t\t\t\t   The user must press ‘y’ for confirmation."
"\tmv -f\t\t\t\tby default, the command will ask the user for confirmation when overwriting the destination file"
"\t\t\t\t\t\t   if that file is write-protected. This flag forcefully overwrites the destination file and deletes the source file."
"\tmv -n\t\t\t\tprevents an existing file from being overwritten if the destination file already exists."
"\tmv -b\t\t\t\tcreates a backup file for the file being overwritten when using the mv command."
"\t\t\t\t\t\t   This new file will have a tilde (~) appended to it.")
